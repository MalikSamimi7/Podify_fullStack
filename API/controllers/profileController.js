...
const getPlaylistAudios = async (req, res) => {
  ...
  if (!playlistAudios) {
    const [autoGeneratedPlaylistAudios] = await AutoGeneratedPlaylist.aggregate(
      aggregationLogic
    );
    return res.send({ list: autoGeneratedPlaylistAudios });
  }

  res.send({ list: playlistAudios });
};
